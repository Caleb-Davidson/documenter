...
function listMarkdownDocsFromDir(rootDir, relativeDir) {
  ...
  for (const entry of entries) {
    ...
    // Normalize to forward slashes so downstream comparisons against literal
    // POSIX-style paths ("templates/", "index.md", template targets) and the
    // index-manifest link matcher work identically on Windows and POSIX.
...
function checkInternalLinks(doc, issues) {
  ...
  walkBodyLines(doc, function (line, lineNumber) {
    ...
    while ((match = markdownLinkPattern.exec(line)) !== null) {
      ...
      // Dangling-link check: for docs-root-relative internal links (documenter's
      // "./" convention), resolve the target file and fail if it does not exist.
      // This catches links left behind after a target doc is deleted or renamed.
      // Links are resolved against the docs root, not the linking file's directory,
      // matching how the index manifest and shell router interpret "./" links.
      if (href.startsWith("./")) {
        const targetFile = resolveDocsLinkTarget(href);
        if (targetFile !== null && !docsFileExists(targetFile)) {
          issues.push(
            `${doc.path}:${lineNumber} link '[${text}](${href})' points to '${targetFile}' which does not exist in docs/.`
          );
        }
      }
    }
...
/**
 * Resolves a docs-root-relative link href ("./...") to its docs-root-relative
 * target file path, dropping any "#anchor" or "?query" suffix. Anchor fragments
 * are intentionally not validated against target headings — only the file part.
 * @param {string} href Link href beginning with "./".
 * @returns {string|null} Docs-root-relative file path, or null when there is no file part (e.g. "./#anchor").
 */
function resolveDocsLinkTarget(href) {
  const withoutPrefix = href.slice(2);
...
/**
 * Reports whether a docs-root-relative path resolves to an existing file under docs/.
 * @param {string} relativePath Docs-root-relative file path (forward slashes).
 * @returns {boolean} True when the path exists and is a regular file.
 */
function docsFileExists(relativePath) {
  try {
    return statSync(join(DOCS_DIR, relativePath)).isFile();
  } catch {
    return false;
  }
}
...
function walkBodyLines(doc, callback) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    callback(maskInlineCode(line), lineNumber);
  }
}

/**
 * Replaces inline code spans (backtick-delimited) with spaces of equal length so
 * line-based content checks skip them, mirroring how fenced blocks are exempted.
 * Preserving length keeps column offsets and line numbers intact. A code span opens
 * on a run of N backticks and closes on the next run of exactly N backticks; an
 * unterminated run is left as-is so it can't swallow the rest of the line.
 * @param {string} line Raw body line.
 * @returns {string} Line with inline code span contents (and their backticks) blanked.
 */
function maskInlineCode(line) {
  ...
  while (i < n) {
    ...
    if (closeEnd >= 0) {
      result += " ".repeat(closeEnd - runStart);
      i = closeEnd;
    } else {
